...
MEM_BUFFER_LIMIT = 4 * 1024 * 1024 * 1024  # 4GB default cap
...
def init_catalog(cat: Path, reserve_percent=10, hash_algo="sha256"):
    ...
    with con:
        ...
        con.execute(
            "INSERT OR REPLACE INTO settings(k,v) VALUES(?,?)", ("hash_algo", hash_algo)
        )
    con.close()
    print(
        f"Catalog initialized at {cat} (reserve {reserve_percent}%, hash {hash_algo})."
    )
...
def add_volume(cat: Path, mount_path: Path, reserve_percent=None):
    ...
    (mount_path / ".rt_volume_manifest").write_text(
        f"volume_id={vol_id}\nstatus=active\n"
    )
    con.close()
...
def ensure_space_or_rotate(
    con: sqlite3.Connection, volume_id: str, mount_path: Path, needed_bytes: int
):
    row = con.execute(
        "SELECT reserve_percent FROM volumes WHERE volume_id=?", (volume_id,)
    ).fetchone()
    ...
    if free_bytes(mount_path) - needed_bytes <= reserve_abs:
        with con:
            con.execute(
                "UPDATE volumes SET status='sealed' WHERE volume_id=?", (volume_id,)
            )
        ...
        raise RuntimeError(
            f"Volume {volume_id} sealed (free space below reserve). Please add a new volume."
        )
...
def _record_volume_index(mount_path: Path, chunk_id: str, size: int):
    ...
    with sqlite3.connect(str(vdb)) as vcon:
        for stmt in [s for s in VOLUME_INDEX_SCHEMA.strip().split(";") if s.strip()]:
            vcon.execute(stmt)
        vcon.execute(
            "INSERT OR IGNORE INTO chunks(chunk_id,size,first_seen_ts) VALUES(?,?,?)",
            (chunk_id, size, int(time.time())),
        )
    try:
        with open(mount_path / ".rt_volume_index.jsonl", "a", encoding="utf-8") as j:
            j.write(
                f'{{"chunk_id":"{chunk_id}","size":{size},"ts":{int(time.time())}}}\n'
            )
    except Exception:
        pass
...
def _compute_file_job(
    ...
    algo: str,
    chunk_bytes: int,
    mem_buffer_limit: int = MEM_BUFFER_LIMIT,
):
    p = Path(src_path)
    ...
    chunks_data: List[bytes] = []  # New: buffered chunks

    if size <= mem_buffer_limit:
        with p.open("rb") as f:
            full_data = f.read()  # Slurp!
        pos = 0
        while pos < size:
            ...
            chunks_data.append(chunk)  # Buffer it
            pos = end
    else:
        # Fallback: stream (original logic)
        with p.open("rb") as f:
            while True:
                ...
                chunks_data.append(b"")  # Placeholder—no buffer for biggies
    ...
    return {
        ...
        "chunks_data": chunks_data,  # New field
        "abs_path": str(p),
    }
...
# ===============================
def _write_chunks_colocated(
    con: sqlite3.Connection,
    abs_path: Path,
    chunk_ids: List[str],
    chunk_sizes: List[int],
    chunks_data: List[bytes] = None,
):
    ...
    needed = sum(
        sz for cid, sz in zip(chunk_ids, chunk_sizes) if cid not in existing_on_target
    )
    if needed > 0:
    ...
    buffered = bool(
        chunks_data
        and len(chunks_data) == len(chunk_ids)
        and all(len(d) > 0 for d in chunks_data)
    )
    if buffered:
        for cid, sz, data in zip(chunk_ids, chunk_sizes, chunks_data):
            if cid in existing_on_target:
                continue
            store_chunk(con, mount_path, vol_id, cid, data)  # Direct from mem!
            con.execute(
                "INSERT OR IGNORE INTO chunks(chunk_id,size) VALUES(?,?)", (cid, sz)
            )
            con.execute(
...
# ===============================
def chunk_file_and_store_single(
    con: sqlite3.Connection,
    src_path: Path,
    algo: str,
    mem_buffer_limit: int = MEM_BUFFER_LIMIT,
    ...
    _write_chunks_colocated(con, p, chunk_ids, chunk_sizes, chunks_data)
...
def scan_and_backup_singlethread(cat: Path, source_dir: Path):
    ...
    for i, p in enumerate(all_paths):
        ...
        if prev and prev[1] == size and prev[2] == mtime_ns:
            ...
            file_hash, chunk_ids = chunk_file_and_store_single(
                con, p, algo, MEM_BUFFER_LIMIT
            )
            with con:
                if prev:
                    file_id = con.execute(
                        "SELECT rowid FROM files WHERE path=?", (rel,)
                    ).fetchone()[0]
                    con.execute(
                        "UPDATE files SET size=?, mtime_ns=?, file_hash=? WHERE path=?",
                    ...
                    con.execute(
                        "INSERT INTO files(path,size,mtime_ns,file_hash) VALUES(?,?,?,?)",
        ...
        eta = (
            int((total_files - processed) / (processed / elapsed))
            if processed > 1
            else 0
        )
        sys.stdout.write(
    ...
    with con:
        con.execute(
            "UPDATE runs SET finished_ts=? WHERE run_id=?", (int(time.time()), run_id)
        )
    rows = con.execute(
        "SELECT change, COUNT(*) FROM run_changes WHERE run_id=? GROUP BY change",
        (run_id,),
    ).fetchall()
    summary = dict(rows)
    con.close()
...
def scan_and_backup_threaded(cat: Path, source_dir: Path, max_workers: int):
    ...
    batch_file_chunks: List[
        Tuple[int, int, str]
    ] = []  # we fill file_id later (per file)
    ...
    def flush_batch_files(files_batch: List[Tuple[str, int, int, str]]):
        ...
        def _do():
            with conw:
                conw.executemany(
                    "INSERT OR REPLACE INTO files(path,size,mtime_ns,file_hash) VALUES(?,?,?,?)",
                    files_batch,
                )

        _retry_locked(_do)

    def record_versioning(
        file_id: int,
        rel: str,
        size: int,
        mtime_ns: int,
    ...
    while processed < submitted:
        ...
        sys.stdout.write(
            f"\rHashing files: {processed}/{total_files} ({pct:.1f}%) | Elapsed: {int(elapsed)}s | ETA: {eta}s"
        )
        sys.stdout.flush()
        ...
        chunk_sizes = res["chunk_sizes"]
        chunks_data = res.get("chunks_data", [])  # New
        # Enforce co-location + write missing chunks (single writer reading the file again)
        try:
            _write_chunks_colocated(conw, abs_path, chunk_ids, chunk_sizes, chunks_data)
        ...
        def _upsert_file_and_chunks():
            with conw:
                prev = conw.execute(
                    "SELECT file_id,size,mtime_ns,file_hash FROM files WHERE path=?",
                    (rel,),
                ).fetchone()
                if prev:
                    file_id = prev[0]
...
def list_changes(cat: Path, run_id: int):
    ...
    for change in ("added", "modified", "deleted"):
        ...
        for (p,) in con.execute(
            "SELECT path FROM run_changes WHERE run_id=? AND change=? ORDER BY path",
            (run_id, change),
        ):
            print(p)
    con.close()
...
def validate_blocks(cat: Path):
        ...
        if not loc:
            ...
            if not path.exists():
                ...
                if actual_size != expected_size:
                    bad += 1
                    print(
                        f"Size mismatch {cid}: expected {expected_size}, got {actual_size}"
                    )
                else:
    ...
    con.close()
    print(
        f"Validation: {good} good, {bad} bad chunks. Your blocks are {'' if bad == 0 else 'mostly'} solid! 💪"
    )
...
def _restore_file(con: sqlite3.Connection, src_path: str, dest_dir: Path, algo: str):
    row = con.execute(
        "SELECT file_id,size,file_hash FROM files WHERE path=?", (src_path,)
    ).fetchone()
    if not row:
    ...
    chunks = list(
        con.execute(
            "SELECT seq,chunk_id FROM file_chunks WHERE file_id=? ORDER BY seq",
            (file_id,),
        )
    )
...
def main():
    ...
    p_restore_file = sp.add_parser(
        "restore", help="Restore a single file to a destination path"
    )
    p_restore_file.add_argument("src_file", type=str)
    p_restore_file.add_argument("dest", type=Path)
    p_restore_folder = sp.add_parser(
        "restore-folder", help="Restore an entire folder subtree"
    )
    p_restore_folder.add_argument("src_folder", type=str)
    p_restore_folder.add_argument("dest_dir", type=Path)
    p_revert = sp.add_parser(
        "revert-to-run",
        help="Materialize files as of a specific run into a dest directory",
    )
    p_revert.add_argument("run_id", type=int)
    p_revert.add_argument("dest_root", type=Path)
    sp.add_parser(
        "rebuild-catalog", help="Rebuild chunks & locations from per-volume indexes"
    )
    args = ap.parse_args()
    cat = args.catalog
    if args.cmd == "init":
